...
interface PoolTableProps {
    poolDatas?: PoolData[];
    auraPools?: AuraPoolData[];
}

export default function AuraPoolTable({poolDatas, auraPools}: PoolTableProps) {
    ...
    const rows = filteredPoolDatas.map(el => {
        const auraMatch = auraPools?.find((pool) => pool.balancerPoolId.toLowerCase() === el.id)
        const tvlCapture = auraMatch ?   (100 / el.totalShares * auraMatch.totalStaked) : 0;
        const percentageFixed = tvlCapture > 100 ? 0 : tvlCapture
            return createData(getShortPoolName(el), el.tokens, el, el.swapFee, el.volumeUSD, el.feesUSD, el.tvlUSD, percentageFixed)
        }
    ...
    return (
        <Box sx={{ width: '100%' }}>
            <Paper sx={{ mb: 2, boxShadow: "rgb(51, 65, 85) 0px 0px 0px 0.5px", }}>
                <TableContainer>
                    <Table
                        //sx={{ minWidth: 750 }}
